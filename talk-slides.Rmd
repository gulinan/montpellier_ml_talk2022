...
- Any **confidence interval** built on it would have **poor coverage**.
...
- The naive CV intervals **fail** due to larger miscoverage rates. 
...
---
# Target of inference: Confidence intervals for $Err_{XY}$
<br>
- <b><span style="color:#067373;">Bates, Hastie, and Tibshirani (2021)</span></b>
develops an **estimator** that **empirically estimate the variance** of $\widehat{Err}^{(CV)}$ across many subsamples and use it to construct confidence intervals for $Err_{XY}$ which are often of the **most interest to the analysts**.
...
---
# Nested cross-validation estimate of MSE
## Nested CV algorithm
- Split data into $K$ folds with $K-1$ building $\mathcal{I}_{(train)}$ and the remaining
...
- For each split j:
 ...
 cross-validation over K-1 folds in $\mathcal{I}_{(train)}$.
  ...
  - Set $b_j := \text{empirical variance of} \quad \{e_i\}_{e \in I_{out}}$ (estimate of (the second term at RHS)).
- Output  $\widehat{MSE} = mean(a_j)-mean(b_j)$. 
- Output $\widehat {Err}^{(NCV)} := mean(\epsilon_j)$.

---
- This algorithm provides us a **point estimate** for prediction error, denoted by $\widehat {Err}^{(NCV)}$, and and **estimate for MSE**, denoted by $\widehat{MSE}$. 
- Since the estimation is done over $K-1$ folds, the actual quantity of interest $MSE_{K,n}$ is estimated with some bias.
- **Theorem 2:**

- Note that the estimator $\widehat {Err}^{(NCV)}$ is targeting the MSE of $\widehat {Err}^{(CV)}$ as an estimate of $Err_{XY}$.
